...
const createOperationKey = ({ method, path: path$4 }) => `${method.toUpperCase()} ${path$4}`;
...
const refToName = ($ref) => {
	...
	const name = path$4[path$4.length - 1];
...
* @returns
*/
const pathToJsonPointer = (path$4) => {
...
function createFnSymbol(plugin, item) {
	const { operation, path: path$4, tags } = item.data;
	...
	if (plugin.config.httpRequests.enabled) {
		...
		plugin.forEach("operation", ({ operation }) => {
			httpRequestStructure.insert({
				data: { operation },
				locations: strategy(operation).map((path$4) => ({
					path: path$4,
					shell
				})),
				source
...
const handlerV1$2 = ({ plugin }) => {
	...
	plugin.forEach("operation", (event) => {
		structure.insert({
			...
			locations: strategy(event.operation).map((path$4) => ({
				path: path$4,
				shell
			})),
			source: source$1
...
const irOperationToAst$1 = ({ getAst, operation, plugin, state }) => {
	...
	if (plugin.config.responses.enabled) {
		if (operation.responses) {
			const { response } = operationResponsesMap(operation);
			if (response) {
				const path$4 = [...fromRef(state.path), "responses"];
				exportAst$1({
					ast: getAst(response, path$4),
					plugin,
					schema: response,
					state,
...
* @deprecated
*/
const operationToId = ({ context, count = 1, id, method, path: path$4, state }) => {
	let result;
	...
	else result = toCase(`${method}-${path$4.replace(/{(.*?)}/g, "by-$1").replace(/[/:+]/g, "-")}`, targetCase);
...
const buildResourceMetadata = (graph, logger) => {
	...
	const getDependencies = (pointer) => {
		...
		if (nodeDependencies?.size) for (const dependency of nodeDependencies) {
			const path$4 = jsonPointerToPath(dependency);
			const type = path$4[path$4.length - 2];
			const name = path$4[path$4.length - 1];
			if (type && name) {
	...
	for (const [pointer, nodeInfo] of graph.nodes) {
		const path$4 = jsonPointerToPath(pointer);
		if (path$4[0] === "components") {
			if (path$4.length === 3) {
				if (path$4[1] === "schemas") {
					const name = path$4[path$4.length - 1];
		...
		if (path$4[0] === "paths") {
			if (path$4.length === 3 && httpMethods.includes(path$4[2])) {
				const method = path$4[path$4.length - 1];
				const operationKey = createOperationKey({
					method,
					path: path$4.slice(1, -1).join("/")
				});
...
const isPathRootSchema = (path$4) => path$4.length === 3 && path$4[0] === "components" && path$4[1] === "schemas" || path$4.length === 2 && path$4[0] === "definitions";
...
*/
const walkSchemas$1 = ({ key, node, parent, path: path$4, visitor }) => {
...
* @param visitor - Function to call for each visited node
*/
const walkSchemas = ({ key, node, parent, path: path$4, visitor }) => {
	if (!node || typeof node !== "object" || node instanceof Array) return;
	const value = node;
...
const buildGraph = (root, logger) => {
	...
	const walk$1 = ({ key, node, parentPointer, path: path$4 }) => {
		if (typeof node !== "object" || node === null) return;
		const pointer = pathToJsonPointer(path$4);
		let deprecated;
		let tags;
...
const getComponentContext = (path$4) => {
	if (path$4.length === 3 && path$4[0] === "components") {
		const type = path$4[1];
...
const removeOriginalSplitSchemas = ({ logger, originalSchemas, spec, split }) => {
	...
	for (const originalPointer of Object.keys(split.mapping)) {
		const path$4 = jsonPointerToPath(originalPointer);
		const name = path$4[path$4.length - 1];
	...
	*/
	const pointerToSchema = (pointer) => {
		if (pointer.startsWith(schemasPointerNamespace)) {
			const path$4 = jsonPointerToPath(pointer);
			if (path$4.length === schemasNamespaceSegments) return path$4[schemasNamespaceSegments - 1] || "";
		}
		return "";
	};
	for (const pointer of graph.nodes.keys()) {
...
const updateRefsInSpec = ({ logger, spec, split }) => {
	...
	const schemasPointerNamespace = specToSchemasPointerNamespace(spec);
	const walk$1 = ({ context, currentPointer, inSchema, node, path: path$4, visited = /* @__PURE__ */ new Set() }) => {
		if (node instanceof Array) node.forEach((item, index) => walk$1({
			context,
			currentPointer,
			inSchema,
			...
			if (isPathRootSchema(path$4)) {
				nextPointer = `${schemasPointerNamespace}${path$4[path$4.length - 1]}`;
			...
			const compContext = getComponentContext(path$4);
...
const formatPath = (path$4) => path$4.map((segment, i) => {
...
const filterSpec$2 = ({ logger, operations, preserveOrder, schemas, spec }) => {
	...
	if (spec.paths) for (const entry of Object.entries(spec.paths)) {
		...
		if (!Object.keys(pathItem).length) delete spec.paths[path$4];
	}
...
const initIrOperation$2 = ({ context, method, operation, path: path$4, state }) => {
	const irOperation = {
		id: operationToId({
			context,
...
const operationToIrOperation$2 = ({ context, method, operation, path: path$4, securitySchemesMap, state }) => {
...
const parsePathOperation$2 = ({ context, method, operation, path: path$4, securitySchemesMap, state }) => {
	if (!context.ir.paths) context.ir.paths = {};
	if (!context.ir.paths[path$4]) context.ir.paths[path$4] = {};
	context.ir.paths[path$4][method] = operationToIrOperation$2({
		context,
		method,
		operation,
		path: path$4,
...
const parseServers$2 = ({ context }) => {
	...
	const path$4 = context.spec.basePath ?? "";
	...
	const servers = schemes.map((scheme) => `${scheme ? `${scheme}://` : ""}${host}${path$4}`).filter(Boolean);
...
const parseV2_0_X = (context) => {
	...
	for (const path$4 in context.spec.paths) {
		if (path$4.startsWith("x-")) continue;
		const pathItem = context.spec.paths[path$4];
...
const initIrOperation$1 = ({ context, method, operation, path: path$4, state }) => {
	const irOperation = {
		id: operationToId({
			context,
			id: operation.operationId,
...
const operationToIrOperation$1 = ({ context, method, operation, path: path$4, securitySchemesMap, state }) => {
...
const parsePathOperation$1 = ({ context, method, operation, path: path$4, securitySchemesMap, state }) => {
	if (!context.ir.paths) context.ir.paths = {};
	if (!context.ir.paths[path$4]) context.ir.paths[path$4] = {};
	if (operation.servers) context.ir.servers = [...context.ir.servers ?? [], ...operation.servers];
	context.ir.paths[path$4][method] = operationToIrOperation$1({
		context,
		method,
		operation,
		path: path$4,
...
const initIrOperation = ({ context, method, operation, path: path$4, state }) => {
	const irOperation = {
		id: operationToId({
			context,
			id: operation.operationId,
...
const operationToIrOperation = ({ context, method, operation, path: path$4, securitySchemesMap, state }) => {
...
const parseOperationObject = ({ context, method, operation, path: path$4, securitySchemesMap, state }) => {
	if (operation.servers) context.ir.servers = [...context.ir.servers ?? [], ...operation.servers];
...
const parsePathOperation = ({ context, method, path: path$4, ...options }) => {
	if (!context.ir.paths) context.ir.paths = {};
	if (!context.ir.paths[path$4]) context.ir.paths[path$4] = {};
	...
	context.ir.paths[path$4][method] = parsed;
};
