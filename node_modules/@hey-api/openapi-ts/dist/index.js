...
};`},useData:true};var nf=()=>{y.registerHelper("camelCase",function(e){return C({case:"camelCase",value:e})}),y.registerHelper("equals",function(e,t,r){return e===t?r.fn(this):r.inverse(this)}),y.registerHelper("ifServicesResponse",function(e,t){return b().plugins["@hey-api/sdk"]?.response===e?t.fn(this):t.inverse(this)}),y.registerHelper("ifdef",function(...e){let t=e.pop();return e.every(r=>!r)?t.inverse(this):t.fn(this)}),y.registerHelper("notEquals",function(e,t,r){return e!==t?r.fn(this):r.inverse(this)}),y.registerHelper("transformServiceName",function(e){return Ie({config:b(),name:e})});},cc=()=>{nf();let e={client:y.template(bp),core:{apiError:y.template(Ip),apiRequestOptions:y.template(Ap),apiResult:y.template(Ep),baseHttpRequest:y.template($p),cancelablePromise:y.template(Dp),httpRequest:y.template(ec),request:y.template(rc),settings:y.template(tc)}};return y.registerPartial("functions/base64",y.template(Kp)),y.registerPartial("functions/catchErrorCodes",y.template(Vp)),y.registerPartial("functions/getFormData",y.template(Wp)),y.registerPartial("functions/getQueryString",y.template(Up)),y.registerPartial("functions/getUrl",y.template(Qp)),y.registerPartial("functions/isBlob",y.template(Xp)),y.registerPartial("functions/isFormData",y.template(zp)),y.registerPartial("functions/isString",y.template(Gp)),y.registerPartial("functions/isStringWithValue",y.template(Jp)),y.registerPartial("functions/isSuccess",y.template(Zp)),y.registerPartial("functions/resolve",y.template(Yp)),y.registerPartial("fetch/getHeaders",y.template(Mp)),y.registerPartial("fetch/getRequestBody",y.template(Fp)),y.registerPartial("fetch/getResponseBody",y.template(Lp)),y.registerPartial("fetch/getResponseHeader",y.template(Bp)),y.registerPartial("fetch/request",y.template(_p)),y.registerPartial("fetch/sendRequest",y.template(Hp)),y.registerPartial("xhr/getHeaders",y.template(nc)),y.registerPartial("xhr/getRequestBody",y.template(sc)),y.registerPartial("xhr/getResponseBody",y.template(oc)),y.registerPartial("xhr/getResponseHeader",y.template(ic)),y.registerPartial("xhr/request",y.template(ac)),y.registerPartial("xhr/sendRequest",y.template(pc)),y.registerPartial("axios/getHeaders",y.template(jp)),y.registerPartial("axios/getRequestBody",y.template(vp)),y.registerPartial("axios/getResponseBody",y.template(qp)),y.registerPartial("axios/getResponseHeader",y.template(kp)),y.registerPartial("axios/request",y.template(wp)),y.registerPartial("axios/sendRequest",y.template(Np)),y.registerPartial("angular/getHeaders",y.template(Op)),y.registerPartial("angular/getRequestBody",y.template(Rp)),y.registerPartial("angular/getResponseBody",y.template(Sp)),y.registerPartial("angular/getResponseHeader",y.template(Tp)),y.registerPartial("angular/request",y.template(Cp)),y.registerPartial("angular/sendRequest",y.template(Pp)),e};var mc=e=>`${e}-end`,Cs=e=>`${e}-length`,lc=e=>`${e}-start`,B={clear:()=>{performance.clearMarks(),performance.clearMeasures();},end:e=>performance.mark(mc(e)),getEntriesByName:e=>performance.getEntriesByName(Cs(e)),measure:e=>performance.measure(Cs(e),lc(e),mc(e)),start:e=>performance.mark(lc(e))},On=class{totalMeasure;constructor({totalMark:t}){this.totalMeasure=B.measure(t);}report({marks:t}){let r=Math.ceil(this.totalMeasure.duration*100)/100,n=this.totalMeasure.name;console.warn(`${n.substring(0,n.length-Cs("").length)}: ${r.toFixed(2)}ms`),t.forEach(s=>{let o=B.measure(s),i=Math.ceil(o.duration*100)/100,a=Math.ceil(o.duration/this.totalMeasure.duration*100*100)/100;console.warn(`${s}: ${i.toFixed(2)}ms (${a.toFixed(2)}%)`);});}};var sf={stringCase:C};var mf={biome:{args:e=>["format","--write",e],command:"biome",name:"Biome (Format)"},prettier:{args:e=>["--ignore-unknown",e,"--write","--ignore-path","./.prettierignore"],command:"prettier",name:"Prettier"}},lf={biome:{args:e=>["lint","--apply",e],command:"biome",name:"Biome (Lint)"},eslint:{args:e=>[e,"--fix"],command:"eslint",name:"ESLint"},oxlint:{args:e=>["--fix",e],command:"oxlint",name:"oxlint"}},uf=({config:e})=>{if(e.output.format){let t=mf[e.output.format];console.log(`\u2728 Running ${t.name}`),(0, Ps.sync)(t.command,t.args(e.output.path));}if(e.output.lint){let t=lf[e.output.lint];console.log(`\u2728 Running ${t.name}`),(0, Ps.sync)(t.command,t.args(e.output.path));}},ff=e=>{let t={bundle:false,name:""};return typeof e.client=="string"?t.name=e.client:e.client&&(t={...t,...e.client}),t},df=e=>{let t={path:""};return typeof e.input=="string"?t.path=e.input:e.input&&e.input.path?t={...t,...e.input}:t={...t,path:e.input},t},fc=e=>{let t={level:"info",path:process.cwd()};return typeof e.logs=="string"?t.path=e.logs:t={...t,...e.logs},t},yf=e=>{let t={clean:true,format:false,lint:false,path:""};return typeof e.output=="string"?t.path=e.output:t={...t,...e.output},t},gf=({pluginConfigs:e,userPlugins:t,userPluginsConfig:r})=>{let n=new Set,s=new Set,o={},i=a=>{if(n.has(a))throw new Error(`Circular reference detected at '${a}'`);if(!s.has(a)){if(n.add(a),!e[a])throw new Error(`\u{1F6AB} unknown plugin dependency "${a}" - do you need to register a custom plugin with this name?`);let m=bn[a],l=r[a];if(l&&m){let f=Object.keys(l).find(d=>d.startsWith("_"));if(f)throw new Error(`\u{1F6AB} cannot register plugin "${a}" - attempting to override a native plugin option "${f}"`)}let u={_dependencies:[],...m,...l};if(u._infer){let f={ensureDependency:d=>{typeof d=="string"&&!u._dependencies.includes(d)&&(u._dependencies=[...u._dependencies,d]);},pluginByTag:d=>{for(let x of t){let h=bn[x];if(h&&h._tags?.includes(d)&&x!==a)return x}}};u._infer(u,f);}for(let f of u._dependencies)i(f);n.delete(a),s.add(a),o[a]=u;}};for(let a of t)i(a);return {pluginOrder:Array.from(s),plugins:o}},hf=e=>{let t={},r=(e.plugins??Rf).map(n=>typeof n=="string"?n:(n.name&&(t[n.name]=n),n.name)).filter(Boolean);return gf({pluginConfigs:{...t,...bn},userPlugins:r,userPluginsConfig:t})},xf=async({inputPath:e,watch:t})=>{let r=new $RefParser,n=getResolvedInput({pathOrUrlOrSchema:e}),s,o,i;if(n.type==="url"){if(t.lastValue){if(i=(await sendRequest({init:{headers:t.headers,method:"HEAD"},url:n.path})).response,!i.ok)return {error:"not-ok",response:i};if(i.status===304)return {error:"not-modified",response:i};if(o===undefined){let l=i.headers.get("ETag");l&&(o=l!==t.headers.get("If-None-Match"),o&&t.headers.set("If-None-Match",l));}if(o===undefined){let l=i.headers.get("Last-Modified");l&&(o=l!==t.headers.get("If-Modified-Since"),o&&t.headers.set("If-Modified-Since",l));}if(o===false)return {error:"not-modified",response:i}}if(i=(await sendRequest({init:{method:"GET"},url:n.path})).response,!i.ok)return {error:"not-ok",response:i};if(s=i.body?await i.arrayBuffer():new ArrayBuffer(0),o===undefined){let m=new TextDecoder().decode(s);o=m!==t.lastValue,t.lastValue=m;}}return o===false?{error:"not-modified",response:i}:{data:await r.bundle({arrayBuffer:s,pathOrUrlOrSchema:undefined,resolvedInput:n})}},bf=e=>{let t={enabled:false,interval:1e3};return typeof e.input.path!="string"||(typeof e.watch=="boolean"?t.enabled=e.watch:typeof e.watch=="number"?t={enabled:true,interval:e.watch}:e.watch&&(t={...t,...e.watch})),t},Of=async e=>{let t;if(e.configFile){let s=e.configFile.split(".");t=s.slice(0,s.length-1).join(".");}let{config:r}=await loadConfig({configFile:t,name:"openapi-ts"});return (Array.isArray(e)?e:Array.isArray(r)?r.map(s=>({...s,...e})):[{...r??{},...e}]).map(s=>{let{base:o,configFile:i="",dryRun:a=false,experimentalParser:c=false,exportCore:m=true,name:l,request:u,useOptions:f=true}=s,d=fc(s);d.level==="debug"&&console.warn("userConfig:",s);let x=df(s),h=yf(s);if(!x.path)throw new Error("\u{1F6AB} missing input - which OpenAPI specification should we use to generate your client?");if(!h.path)throw new Error("\u{1F6AB} missing output - where should we generate your client?");let T=ff(s);if(T.name&&!xp.includes(T.name))throw new Error("\u{1F6AB} invalid client - select a valid client value");f||console.warn("\u2757\uFE0F Deprecation warning: useOptions set to false. This setting will be removed in future versions. Please migrate useOptions to true https://heyapi.dev/openapi-ts/migrating.html#v0-27-38"),h.path=ce.resolve(process.cwd(),h.path);let O=So({...hf(s),base:o,client:T,configFile:i,dryRun:a,experimentalParser:c,exportCore:I(T)?m:false,input:x,logs:d,name:l,output:h,request:u,useOptions:f,watch:bf({...s,input:x})});return d.level==="debug"&&console.warn("config:",O),O})};async function cA(e){let t=[];try{B.start("createClient"),B.start("config"),t=await Of(e),B.end("config"),B.start("handlebars");let r=cc();B.end("handlebars");let n=async({config:a,watch:c})=>{let m=a.input.path,l=c||{headers:new Headers,lastValue:void 0};B.start("spec");let{data:u,error:f,response:d}=await xf({inputPath:m,watch:l});if(B.end("spec"),f&&!c)throw new Error(`Request failed with status ${d.status}: ${d.statusText}`);let x,h;if(u){if(console.log(c?`\u23F3 Input changed, generating from ${m}`:`\u23F3 Generating from ${m}`),B.start("parser"),a.experimentalParser&&!I(a)&&!w(a)&&(h=ga({config:a,spec:u})),!h){let T=ya({openApi:u});x=Sa(T,a);}if(B.end("parser"),B.start("generator"),h?await gi({context:h}):x&&await yi({client:x,openApi:u,templates:r}),B.end("generator"),B.start("postprocess"),!a.dryRun){uf({config:a});let T=process.env.INIT_CWD?`./${ce.relative(process.env.INIT_CWD,a.output.path)}`:a.output.path;console.log(`\u{1F680} Done! Your output is in ${T}`);}B.end("postprocess");}return a.watch.enabled&&typeof m=="string"&&setTimeout(()=>{n({config:a,watch:l});},a.watch.interval),h||x},o=(await Promise.all(t.map(a=>n({config:a})))).filter(a=>!!a);B.end("createClient");let i=t[0];return i&&i.logs.level==="debug"&&new On({totalMark:"createClient"}).report({marks:["config","openapi","handlebars","parser","generator","postprocess"]}),o}catch(r){let n=t[0];if(!(n?n.dryRun:e?.dryRun)){let o=n?.logs??fc(e);if(o.level!=="silent"){let i=`openapi-ts-error-${Date.now()}.log`,a=ce.resolve(process.cwd(),o.path??"");Qe(a);let c=ce.resolve(a,i);pe.writeFileSync(c,`${r.message}
...
export { cA as createClient, Rf as defaultPlugins, mA as defineConfig, sf as utils };
